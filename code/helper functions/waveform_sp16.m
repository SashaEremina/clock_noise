...
for i=1:cellnum
    [troughs, locs1]=findpeaks(-MY_smooth(time_pt_zero+5:end,i),'MinPeakDistance',10); %skip the 1st trough
    troughs=-troughs;
    wave(i).troughs=troughs;
    wave(i).tlocs=locs1;
    t_correct=times_full(time_pt_zero:end); %times_full instead of time_adjusted
    wave(i).ttimes=t_correct(locs1+5);
    wave(i).tperiod=nanmean(diff(times_full(locs1+5)));

    if wave(i).tperiod>26
        if find(diff(times_full(locs1))>40)>0 %a skipped peak -> re-do the analysis
            [troughs, locs1]=findpeaks(-MY_smooth(time_pt_zero+5:end,i),'MinPeakDistance',14); %need to adjust
    ...
    [peaks, locs2]=findpeaks(MY(time_pt_zero:end,i),'MinPeakDistance',10);
    wave(i).cellnum=i;
    wave(i).peaks=peaks;
    wave(i).plocs=locs2;
    ...
    wave(i).pperiod=nanmean(diff(times_full(locs2)));
    %wave(i).pperiod_std=std(diff(times_full(locs2)));
    
      if wave(i).pperiod>26
        if find(diff(times_full(locs2))>40)>0 %a skipped peak -> re-do the analysis
            [peaks, locs2]=findpeaks(MY(time_pt_zero:end,i),'MinPeakDistance',14); %need to adjust
            ... for non-linear time vector to around 10 %212 for 0.75 acquisition rate
